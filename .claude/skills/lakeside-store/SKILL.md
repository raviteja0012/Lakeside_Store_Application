---
name: lakeside-store
description: Build, extend, and maintain the Lakeside Dry Goods store operations platform, a capture-first receiving app for a Canadian general store. Use this whenever the work touches the Lakeside app, the store-ops build, the receiving form or capture loop, the vendor and order ledger, invoice and payment tracking, the document-extraction or reorder or ask-your-store agents, the Postgres schema, the color and form design system, or Canadian tax and privacy rules for this store. Trigger even when the request only says the store app, Lakeside, Ravi's store, the receiving screen, or a store-ops feature, without naming this skill. Apply the locked architecture and the output rules here on every such task.
---
...
Store facts (confirm the first one):
- Operates as Robinsons General Store, 1062 Main Street, Dorset, Ontario, P0A 1E0 per the pesticide licence. The earlier PRD said Atikokan. Confirm legal name and address. Tax is unaffected, both are Ontario at 13 percent HST.
...
The full plan with evidence, pricing, and citations is in lakeside_build_spec_v2.md. Read it for depth. This skill is the working summary plus the rules to apply on every task.
...
lakeside-store/
  ...
  src/lib/types.ts
  .claude/skills/lakeside-store/  this skill, so it travels with the code
```
...
- lakeside_build_spec_v2.md in the repo, the full plan with evidence, pricing, and caveats.
