---
name: robinsons-store-ops
description: Build, extend, and maintain the Robinsons General Store operations platform, a capture-first receiving app for a Canadian general store. Use this whenever the work touches the Robinsons app, the store-ops build, the receiving form or capture loop, the vendor and order ledger, invoice and payment tracking, the document-extraction or reorder or ask-your-store agents, the Postgres schema, the color and form design system, or Canadian tax and privacy rules for this store. Trigger even when the request only says the store app, Robinsons, Ravikiran's store, the receiving screen, or a store-ops feature, without naming this skill. Apply the locked architecture and the output rules here on every such task.
---
...
robinsons-store/
  ...
  src/lib/types.ts
  .claude/skills/robinsons-store-ops/  this skill, so it travels with the code
```
